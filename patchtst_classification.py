#!/usr/bin/env python3
"""
PatchTST 분류 모델 학습 스크립트
Time-Series-Library의 PatchTST 모델을 사용한 분류 학습
"""
...
class PatchTSTConfig:
    ...
    def __init__(self):
        ...
        # 모델 하이퍼파라미터
        self.e_layers = 3      # 인코더 레이어 수
        self.d_model = 128     # 모델 차원
        self.d_ff = 256        # Feed-forward 차원
        ...
        # PatchTST 특화 설정
        self.patch_len = 16    # 패치 길이
        self.stride = 8        # 패치 스트라이드
        ...
        # Wandb 설정
        self.use_wandb = True
        self.wandb_project = 'patchtst-classification'
...
class PatchTSTTrainer:
    ...
    def init_wandb(self):
        """Wandb 초기화"""
        run_name = f"patchtst_{int(time.time())}"
    ...
    def train_epoch(self, train_loader):
        ...
        for batch_x, batch_y in train_loader:
            ...
            # PatchTST는 x_mark_enc를 None으로 전달
...
def main():
    """메인 함수"""
    # 명령행 인자 파싱
    parser = argparse.ArgumentParser(description='PatchTST 분류 모델 학습')
    ...
    parser.add_argument('--e_layers', type=int, default=3,
                       help='인코더 레이어 수 (default: 3)')
    parser.add_argument('--d_model', type=int, default=128,
                       help='모델 차원 (default: 128)')
    parser.add_argument('--d_ff', type=int, default=256,
                       help='Feed-forward 차원 (default: 256)')
    ...
    parser.add_argument('--patch_len', type=int, default=16,
                       help='패치 길이 (default: 16)')
    parser.add_argument('--stride', type=int, default=8,
                       help='패치 스트라이드 (default: 8)')
    ...
    parser.add_argument('--no_wandb', dest='use_wandb', action='store_false',
                       help='Wandb 로깅 비활성화')
    parser.add_argument('--wandb_project', type=str, default='patchtst-classification',
                       help='Wandb 프로젝트 이름 (default: patchtst-classification)')
    
    args = parser.parse_args()
    
    print("PatchTST 분류 모델 학습 시작")
    print("=" * 50)
    ...
    print(f"PatchTST 설정: patch_len={args.patch_len}, stride={args.stride}")
    ...
    print("예측 결과가 'patchtst_submission.csv'에 저장되었습니다.")
    
    # Wandb에 CSV 업로드
    if config.use_wandb:
        trainer.upload_csv('patchtst_submission.csv', 'patchtst_submission')
