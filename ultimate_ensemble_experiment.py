...
class PairwiseBinaryClassifier(nn.Module):
    def __init__(self, input_dim, embed_dim=32, dropout=0.3):
        super().__init__()
        self.classifier = nn.Sequential(
            nn.Linear(input_dim, embed_dim),
            nn.ReLU(),
...
# OVR Training
# ----------------------------
def train_ovr_classifiers(X_train, y_train, X_val, y_val, device, target_classes=[0, 9, 15]):
    """OVR 이진 분류기 학습"""
    ovr_models = {}
    
    for c in target_classes:
        print(f"  Training OVR for class {c}...")
        
        y_binary_train = (y_train == c).astype(int)
        y_binary_val = (y_val == c).astype(int)
        ...
        model = OVRBinaryClassifier(X_train.shape[1]).to(device)
        criterion = nn.BCEWithLogitsLoss()
        optimizer = optim.Adam(model.parameters(), lr=1e-3)
        
        best_f1 = 0
        best_state = model.state_dict()  # 초기 상태 저장
        patience = 20
        wait = 0
        ...
        for epoch in range(50):
            ...
            model.eval()
            with torch.no_grad():
                val_outputs = model(torch.tensor(X_val, dtype=torch.float32).to(device)).squeeze()
                val_probs = torch.sigmoid(val_outputs).cpu().numpy()
                val_preds = (val_probs > 0.5).astype(int)
                
                # F1 계산 (0일 경우 처리)
                if y_binary_val.sum() == 0 or val_preds.sum() == 0:
                    f1 = 0.0
                else:
                    f1 = f1_score(y_binary_val, val_preds, average='binary', zero_division=0)
            
            # 10 epoch마다 또는 개선 시 출력
            if (epoch + 1) % 10 == 0 or f1 > best_f1:
                print(f"    Epoch {epoch+1}/50 - Loss: {train_loss/len(train_loader):.4f}, Val F1: {f1:.4f}, Best F1: {best_f1:.4f}")
...
# Pairwise Training
# ----------------------------
def train_pairwise_classifiers(X_train, y_train, X_val, y_val, device, pairs=[(0, 15), (0, 9), (15, 9)]):
    """쌍대 이진 분류기 학습"""
    pairwise_models = {}
    
    for c1, c2 in pairs:
        print(f"  Training Pairwise for classes {c1} vs {c2}...")
        
        # 해당 클래스만 필터링
        mask_train = (y_train == c1) | (y_train == c2)
        ...
        y_pair_train = (y_train[mask_train] == c2).astype(int)  # c2=1, c1=0
        
        X_pair_val = X_val[mask_val]
        y_pair_val = (y_val[mask_val] == c2).astype(int)
        ...
        model = PairwiseBinaryClassifier(X_train.shape[1]).to(device)
        criterion = nn.BCEWithLogitsLoss()
        optimizer = optim.Adam(model.parameters(), lr=1e-3)
        ...
        for epoch in range(40):
            ...
            # 10 epoch마다 또는 개선 시 출력
            if (epoch + 1) % 10 == 0 or f1 > best_f1:
                print(f"    Epoch {epoch+1}/40 - Loss: {train_loss/len(train_loader):.4f}, Val F1: {f1:.4f}, Best F1: {best_f1:.4f}")
...
def run_ultimate_experiment(args):
    ...
    # ========== OVR & Pairwise 보정 ==========
    print(f"\n{'='*80}")
    print("OVR & Pairwise 보정")
    print(f"{'='*80}")
    
    # OOF 데이터를 사용하여 OVR 학습 (전체 데이터 대신)
    # 이미 메타 스태킹에서 사용한 OOF 확률을 활용
    print("\nOVR Classifiers 학습 (OOF 기반)...")
    ...
    for c in [0, 9, 15]:
        ...
        train_dataset = TensorDataset(
            torch.tensor(X_ovr_train, dtype=torch.float32),
            torch.tensor(y_ovr_train, dtype=torch.float32)
        )
        train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
        ...
        for epoch in range(50):
            ...
            model.eval()
            with torch.no_grad():
                val_outputs = model(torch.tensor(X_ovr_val, dtype=torch.float32).to(device)).squeeze()
                val_probs = torch.sigmoid(val_outputs).cpu().numpy()
                val_preds = (val_probs > 0.5).astype(int)
                
                if y_ovr_val.sum() == 0 or val_preds.sum() == 0:
                    f1 = 0.0
                else:
                    f1 = f1_score(y_ovr_val, val_preds, average='binary', zero_division=0)
        ...
        if best_state is not None:
            model.load_state_dict(best_state)
        ovr_models[c] = model
        print(f"    Class {c} OVR 학습 완료 (Best F1: {best_f1:.4f})")
    
    # Pairwise 모델 학습
    print("\nPairwise Classifiers 학습...")
    pairwise_models = {}
    ...
    for c1, c2 in [(0, 15), (0, 9), (15, 9)]:
        ...
        model = PairwiseBinaryClassifier(X.shape[1]).to(device)
        criterion = nn.BCEWithLogitsLoss()
        optimizer = optim.Adam(model.parameters(), lr=1e-3)
    ...
    # OVR 예측
    print("\nOVR 예측 적용...")
    ovr_probs = np.zeros((len(X_test), num_classes))
    for c, model in ovr_models.items():
        model.eval()
        with torch.no_grad():
    ...
    # OVR 보정
    alpha_ovr = {0: 0.35, 9: 0.20, 15: 0.35}
    for c, alpha in alpha_ovr.items():
        if c in ovr_models:
            test_probs_final[:, c] = (1 - alpha) * test_probs_final[:, c] + alpha * ovr_probs[:, c]
    
    # Pairwise 보정 (Top-2에 해당 쌍이 있을 때만)
    for i in range(len(test_probs_final)):
        top2_idx = np.argsort(test_probs_final[i])[-2:][::-1]
        top2_set = set(top2_idx)
        
        # 0-15 쌍
        if top2_set == {0, 15} and (0, 15) in pairwise_probs:
            prob_15 = pairwise_probs[(0, 15)][i]
            prob_0 = 1 - prob_15
            test_probs_final[i, 0] = 0.6 * test_probs_final[i, 0] + 0.4 * prob_0
            test_probs_final[i, 15] = 0.6 * test_probs_final[i, 15] + 0.4 * prob_15
        
        # 0-9 쌍
        elif top2_set == {0, 9} and (0, 9) in pairwise_probs:
            prob_9 = pairwise_probs[(0, 9)][i]
            prob_0 = 1 - prob_9
            test_probs_final[i, 0] = 0.6 * test_probs_final[i, 0] + 0.4 * prob_0
            test_probs_final[i, 9] = 0.6 * test_probs_final[i, 9] + 0.4 * prob_9
        
        # 15-9 쌍
        elif top2_set == {15, 9} and (15, 9) in pairwise_probs:
            prob_9 = pairwise_probs[(15, 9)][i]
            prob_15 = 1 - prob_9
            test_probs_final[i, 15] = 0.6 * test_probs_final[i, 15] + 0.4 * prob_15
            test_probs_final[i, 9] = 0.6 * test_probs_final[i, 9] + 0.4 * prob_9
    ...
    if use_meta:
        meta_info = {
            ...
            "oof_f1_simple_weighted": float(simple_weighted_f1),
            "oof_f1_meta_stacking": float(oof_meta_f1),
            "improvement": float(oof_meta_f1 - simple_weighted_f1),
            "meta_features": {
