@echo off
REM TimesNet 정규화 방법 및 하이퍼파라미터 실험 스크립트 (Windows)
REM Val Macro F1 기준으로 최적 파라미터 찾기

echo ==========================================
echo TimesNet 정규화 방법 및 하이퍼파라미터 실험
echo ==========================================

REM 실험 결과 저장 디렉토리 생성
if not exist experiments mkdir experiments
if not exist results mkdir results

REM 실험 시작 시간 기록
echo 실험 시작 시간: %date% %time%

REM 결과 저장 파일
set RESULTS_FILE=results\experiment_results.csv
...
set total_experiments=60
echo 총 실험 수: %total_experiments%

REM 정규화 방법들
set SCALING_METHODS=standard minmax robust sequence

REM 각 정규화 방법에 대해 실험
for %%s in (%SCALING_METHODS%) do (
    echo.
    echo ==========================================
    echo 정규화 방법: %%s
    echo ==========================================
    ...
    REM 하이퍼파라미터 조합들
    set PARAMS=2 32 64 3 4 0.001 64 2 64 128 5 6 0.001 64 3 64 128 5 6 0.001 64 2 128 256 5 6 0.001 64 2 64 128 5 6 0.0005 64 2 64 128 5 6 0.002 64 2 64 128 5 6 0.005 64 2 64 128 5 6 0.001 32 2 64 128 5 6 0.001 128 2 64 128 5 6 0.001 256 1 32 64 3 4 0.001 64 4 64 128 5 6 0.001 64 2 32 64 3 4 0.001 32 2 96 192 7 8 0.001 64
    
    REM 각 하이퍼파라미터 조합에 대해 실험
    for %%p in (%PARAMS%) do (
...
# 실험 실행
val_macro_f1, status = run_experiment('%%s', %%p)
...
"
        ...
        echo 결과: Val Macro F1 = !val_macro_f1!, 상태 = !status!
    )
)
...
echo.
echo 실험 결과가 %RESULTS_FILE%에 저장되었습니다.
echo 최적 파라미터로 최종 모델을 학습하려면:
echo python timesnet_classification.py
